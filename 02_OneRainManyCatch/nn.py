...
def deconv_valid(name, x, kernel_shape, stride, output_shape, channel, initializer, af):
    ...
    if af is None:
        tf.add(tf.nn.conv2d_transpose(x, w, output_shape=(batch_size, output_shape[0], output_shape[1], channel[1]), strides=(1, stride[0], stride[1], 1), padding='VALID'), b, name=name), w, b
    else:
