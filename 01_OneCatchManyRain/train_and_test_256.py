...
dem_process_func = load_data.generate_features # load_data.rescale_dem_negative_positive # load_data.rescale_dem_zero_one, load_data.generate_features
...
model_path = "model2"
model_name = "portugal_256_features" # nbc refers to normalize by catchment
...
def run_test(name, dem_array, rain_pattern_vector, target_array=None, random_patch = False, model_id="/-79", batch_size=32, img_height = 20, save_npy=False):
    ...
    if save_npy:
        np.save(test_path + "/"+name+"_prediction.npy",output)
        
    # ===================render test results===================
    render_pred = render(output, plt.cm.terrain, 0, 3, mask_indice)
    cv2.imwrite(test_path + "/"+name+"_prediction.png",cv2.cvtColor(render_pred,cv2.COLOR_RGBA2BGRA))
    ...
    if target_array is not None:
        render_truth = render(target_array, plt.cm.terrain, 0, 3, mask_indice)
        cv2.imwrite(test_path + "/"+name+"_truth.png",cv2.cvtColor(render_truth,cv2.COLOR_RGBA2BGRA))
        
        render_err = render(target_array - output, plt.cm.seismic, -2, 2, mask_indice)
        cv2.imwrite(test_path + "/"+name+"_error.png",cv2.cvtColor(render_err,cv2.COLOR_RGBA2BGRA))
        
        err = target_array[mask_indice_inv] - output[mask_indice_inv]

        plt.hist(err,50,density=True)
        plt.savefig(test_path + "/"+name+"_error_hist.png",bbox_inches="tight")
        plt.close()
        
        plt.hist(err[np.abs(err)>0.1],50,density=True)
        plt.savefig(test_path + "/"+name+"_error_hist_0.1.png",bbox_inches="tight")
        plt.close()
    ...
    cv2.imwrite(test_path + "/"+name+"_conf.png",cv2.cvtColor(render_stddev,cv2.COLOR_RGBA2BGRA))
    ...
    if target_array is None:    
        ...
        # plt.colorbar()
        plt.savefig(test_path + "/"+name+".png",bbox_inches="tight")
        plt.close()
    else:
