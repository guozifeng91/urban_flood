...
def build_and_train_old():
    '''
    this function shuffles the patches and takes 10% of them as testing data

    it is deprecated as here we are interested that with the same terrain and different pattern. can some pattern be generalized by learning from others
    '''
    batch_size = 8
    epoche = 500
    ...
    patch_terrain, patch_water_levels, rain_patterns, coords, img_shape = ld.load_terrain(path_terrain, path_water_level, path_rain_pattern, 1000, IMG_SIZE)

    train_terrain = patch_terrain[:900]
    train_water_level = patch_water_levels[:900]

    test_terrain = patch_terrain[900:]
    test_water_level = patch_water_levels[900:]
    ...
    with tf.Session() as sess:
        ...
        for e in range(epoche):
            ...
            id_patch = np.arange(train_terrain.shape[0])
            id_pattern = id_patch % len(rain_patterns)

            np.random.shuffle(id_patch)
            np.random.shuffle(id_pattern)

            for i in range(0, train_terrain.shape[0], batch_size):
                # select random patches by id_patch[i:i+batch_size]
                # select random pattern for each patch, by
                # id_pattern[i:i+batch_size]

                sess.run(train_op, feed_dict={phs["x1"]:train_terrain[id_patch[i:i + batch_size]], 

                                              phs["x2"]:rain_patterns[id_pattern[i:i + batch_size]], 

                                              phs["y"]:train_water_level[id_patch[i:i + batch_size],id_pattern[i:i + batch_size]]})

            print("epoche",e, "loss",get_loss(sess,ls,phs,test_terrain,test_water_level, rain_patterns))

            if e % 50 == 49:
                saver.save(sess, "data/model/seg_net_portugal/",e)
...
def test_model(path_model):
    ...
    print("min water level", np.min(test_water_level))

    all_water_level = np.load(path_water_level)
    ...
    with tf.Session() as sess:
        ...
        id_patch = np.arange(test_terrain.shape[0])
        np.random.shuffle(id_patch)

        for i in range(len(rain_patterns)):
            id_pattern = np.ones(num_per_test,dtype = np.uint16) * i
            print("pattern",i)

            plt.figure(figsize=(20,10))
            plt.imshow(all_water_level[i],cmap=plt.cm.jet,vmin=0,vmax=2)
            plt.colorbar()
            plt.savefig("data/images/pattern_" + str(i) + "_0.png")

            for r in range(round_per_test):
                id_patch_test = id_patch[r * num_per_test:(r + 1) * num_per_test]
                predict = sess.run(ls["prediction"], feed_dict={phs["x1"]:test_terrain[id_patch_test], phs["x2"]:rain_patterns[id_pattern]})
                truth = test_water_level[id_patch_test, id_pattern]
                plot_img(test_terrain[id_patch_test],truth,predict, rain_patterns[i], num_per_test,"data/images/pattern_" + str(i) + "_" + str(r + 1) + ".png")


def plot_img(x,y,p,rain,batch_size,filename=None):
    '''
    plot terrain image, the ground truth and the prediction
    '''
    plt.figure(figsize=((1 + batch_size) / 3 * 10,10))
    
    col_num = batch_size + 1
    for i in range(batch_size):
        # the image
        plt.subplot(3,col_num,i + 2)
        plt.imshow(x[i,:,:,0],cmap=plt.cm.jet,vmin=0,vmax=1) # terrain
        plt.colorbar()
        # the label
        plt.subplot(3,col_num,i + col_num + 2)
        plt.imshow(y[i],cmap=plt.cm.jet,vmin=0,vmax=2)
        plt.colorbar()
        # the prediction
        plt.subplot(3,col_num,i + 2 * col_num + 2)
        plt.imshow(p[i],cmap=plt.cm.jet,vmin=0,vmax=2)
        plt.colorbar()

    plt.subplot(3,col_num, 1)
    plt.plot(rain)
    #plt.text(0,0.5,"height")
    #plt.axis('off')

    plt.subplot(3,col_num, col_num + 1)
    plt.text(0,0.5,"truth")
    plt.axis('off')

    plt.subplot(3,col_num, 2 * col_num + 1)
...
def test_model_assemble(path_model):
    ...
    with tf.Session() as sess:
        ...
        for pid in range(len(rain_patterns)):
            ...
            colorbar(ax2.imshow(truth_water_level,cmap=plt.cm.jet,vmin=0,vmax=2), cax=make_axes_locatable(ax2).append_axes("right", size="4%", pad="2%"))

            ax3.set_title(SAMPLE_PATTERN[pid] + "prediciton")
            colorbar(ax3.imshow(predict_water_level,cmap=plt.cm.jet,vmin=0,vmax=2), cax=make_axes_locatable(ax3).append_axes("right", size="4%", pad="2%"))

            ax4.set_title("error")
            colorbar(ax4.imshow(error,vmin=0,vmax=0.5), cax=make_axes_locatable(ax4).append_axes("right", size="4%", pad="2%"))

            plt.savefig("data/images/" + SAMPLE_PATTERN[pid] + ".png")
...
def test_model_assemble_anyrain(path_model, rains=None):
    ...
    with tf.Session() as sess:
        ...
        for pid in range(len(rain_patterns)):
            ...
            ax1.set_ylim(0,160)
            ax1.plot(rain_patterns[pid])
            colorbar(ax2.imshow(predict_water_level,cmap=plt.cm.jet,vmin=0,vmax=2), cax=make_axes_locatable(ax2).append_axes("right", size="4%", pad="2%"))

            plt.savefig("data/images/" + "rnd_" + str(pid) + ".png")

build_and_train()
#test_model("data/model/seg_net_portugal/-499")
#test_model_assemble("data/model/seg_net_portugal/-499")

#rain_pattern = np.random.rand(20,12)*160
#test_model_assemble_anyrain("data/model/seg_net_portugal/-499", rain_pattern)